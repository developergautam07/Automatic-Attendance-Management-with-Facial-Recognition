...
def contact():
    mess._show(title='Contact us',
               message="Please contact us on: 'nilangshumajhi98@gmail.com'")
...
def TrackImages():
    ...
    # Adjust the webcam window size and set appropriate flags
    ...
    cam.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # Set webcam width
    cam.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # Set webcam height
    ...
    def save_and_update():
        ...
        filename = f"Attendance_List_{date_time}.csv"
        ...
        with open(filename, 'r') as file:
            ...
            for i, line in enumerate(lines[1:], start=1):
                if i % 2 != 0:
                    ...
                    iidd = line[0] + '   '
                    tv.insert('', 0, text=iidd, values=(line[2], line[4], line[6]))
        file.close()
    ...
    def display_timer():
        ...
        while time.time() < end_time:
            ...
            im = np.zeros((100, 300, 3), np.uint8)
            cv2.putText(im, f"Time Remaining: {remaining_time} seconds", (10, 60), font, 0.7, (255, 255, 255), 2)
            cv2.imshow('Taking Attendance', im)
            cv2.waitKey(1000)  # Update every 1 second
        close_cam()

    threading.Thread(target=display_timer).start()
